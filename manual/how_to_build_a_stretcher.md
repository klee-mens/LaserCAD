...
So, let's try to make a more complex setup like an Oeffner stretcher for chirped
...
use some pre-assembled modules like the RoofTop_Mirror and Stripe_mirror.
...
and the sources given above if you have questions.
```python
...
The next thing is to adjust the grating normal. Maybe this step will be included
in the grating logic one day, now we have to do it manually. We want our grating
...
```
Ok, to be honest we cheated a little bit here. To my knowledge, there is no
...
section (for example, if the aperture exceeds 6 inches, this mount would fail).

The same is true for the convex stripe mirror.
```python
...
elements. Here, we set the light source explicitly and redefine the
...
After that, we just set the mirrors on the axis to form the Oeffner triplet.
The separation is the distance between the center of curvature of the big sphere
...
In fact, for most modern laser applications, the beams are not in the visible
spectrum. So the color is kind of arbitrary chosen to be red in LaserCAD, but
it can be changed. For the stretcher, we would like to see 2 different things:
1. We do not need 3-dimensional beams; the 1D rays would be more clear here.
...
So, we need to define our own rainbow light source.
```python
...
create a list of rays that covers the whole spectral bandwidth and use the
...
With the following lines, we can visualize our light source and the action of the
grating:
```python
...
wavelength. Then we propagate an arbitrary distance to not directly start with
the grating but have some space before so that the input ray has a finite length
and is visible. The helper is adjusted to this new position and then shifted down,
so the input ray will pass underneath the convex stripe mirror. Tricks like
this are often used in multipass systems in LaserCAD. The new-shifted helper
composition is added to the stretcher, and the sequence is adjusted so that the
beam will correctly run through the telescope and hit the grating again. Here
...
Stretcher.draw()
```
The rooftop mirror is a predefined special kind of periscope, thats code can
be found in the modules folder. (More correctly, the folder is a Python modules, so
it should be the moduls module, ...anyway). In short, it consists of 2 invisible
mirrors at 90 degree angle and a non-interacting component that carries the
shape. The periscope's height sets the difference between the input and output beam
