...
class Off_Axis_Parabola(Opt_Element):
  def __init__(self, name="NewFocusOAP", reflected_focal_length=2*inch, 
               angle=90, theta=0, colim=False, **kwargs):
  ...
  def set_geom(self, geom):
    super().set_geom(geom)
    if self.colim:      
      x0,y0,z0 = self.get_coordinate_system()
      self.rotate(vec=z0, phi=-np.pi/2)
      self.rotate(vec=self.normal, phi=np.pi)
      self.rotate(vec=y0, phi=self._theta/180*np.pi)
    else:
      self.rotate(vec=self.normal, phi=self._theta/180*np.pi)
  ...
  def intersection(self, ray):
    ...
    b1 = -np.dot(ns,n0)/curv + 2*np.dot(ns,y0)*np.dot(rel,y0) + 2*np.dot(ns,z0)*np.dot(rel,z0)
