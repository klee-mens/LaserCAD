...
for wavel in wavels:
    a =(cavity_and_stretcher(C_radius=C_radius,vertical_mat=True,want_to_draw=False,
                              roundtrip = roundtrip,centerlamda=wavel,s_shift=StripeM_shift,Tele_added=False))
    matA.append(a[0][0])
    matB.append(a[0][1])
    matC.append(a[1][0])
    matD.append(a[1][1])
...
plt.legend(['vertical','horizontal'],loc = 'center left', fontsize=fs-2)
...
a1.tick_params(axis='y', labelsize=fs)
plt.xlabel("wavelength (nm)", fontsize=fs)
plt.ylabel("matrix number B", fontsize=fs)
plt.legend(['vertical','horizontal'],loc = 'center right', fontsize=fs-2)
...
a1.tick_params(axis='y', labelsize=fs)
plt.xlabel("wavelength (nm)", fontsize=fs)
plt.ylabel("matrix number C", fontsize=fs)
