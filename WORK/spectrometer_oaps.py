...
from LaserCAD.basic_optics import ThickBeamsplitter, Component, Composed_Mount
...
cool_rod.pos = (0,COOLING_ROD_SHIFT,0)
...
thickness_crystal = 5
refractive_index_crystal = 1.3
delta = thickness_crystal * refractive_index_crystal
aperture_crystal = 12
crystal = Crystal(width=6, height=6, thickness=thickness_crystal, name="Sample")
crystal = ThickBeamsplitter(angle_of_incidence=0, thickness=thickness_crystal, 
                            refractive_index=refractive_index_crystal)
crystal.aperture = 12
crystal.Mount.invisible = True
# from LaserCAD.moduls.transmission_disk import Transmission_Disk

# crystal = Transmission_Disk(name="YbCaF2", refractive_index=refractive_index_crystal,
                            # AOI=0, thickness=thickness_crystal, aperture=aperture_crystal)
# crystal.non_opticals[0].Mount.invisible = True


spectro = Composition(name="Spectro")
spectro.set_light_source(fiber1)

fibre_holder = Component()
fibre_holder.set_mount(Stages_Mount(basic_mount=Composed_Mount(["KS1", "1inch_post"])))
spectro.add_on_axis(fibre_holder)

spectro.propagate(white_light_focus*1-delta)
...
# spectro.add_supcomposition_on_axis(crystal)
# spectro.propagate(oap_focus)
...
spectro.pos = (200, 57-210 + COOLING_ROD_SHIFT, 110)
...
pump_geom = spectro.compute_beams()[4].get_geom()

pump_comp = Composition(name="PumpLine")
pump_comp.set_light_source(pump_fibre)
pump_comp.set_geom(pump_geom)
pump_comp.rotate(vec=(0,0,1), phi=30*np.pi/180)
pump_comp.propagate(pump_focal)
pump_comp.add_on_axis(pump_teles_lens2)
pump_comp.propagate(pump_focal*1.5)
pump_comp.add_on_axis(pump_teles_mirror)
pump_comp.propagate(pump_focal*0.5)
pump_comp.add_on_axis(pump_teles_lens1)
pump_comp.propagate(pump_focal)

fibre_holder_pump = Component()
fibre_holder_pump.set_mount(Stages_Mount(basic_mount=Composed_Mount(["KS1", "1inch_post"])))

pump_comp.add_on_axis(fibre_holder_pump)
