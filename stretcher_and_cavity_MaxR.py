...
# matD_h = []
# for wavel in wavels:
#     a =(cavity_and_stretcher(C_radius=C_radius,vertical_mat=True,want_to_draw=False,roundtrip = roundtrip,centerlamda=wavel,s_shift=StripeM_shift))
#     matA.append(a[0][0])
#     matB.append(a[0][1])
#     matC.append(a[1][0])
#     matD.append(a[1][1])
#     a =(cavity_and_stretcher(C_radius=C_radius,vertical_mat=False,want_to_draw=False,roundtrip = roundtrip,centerlamda=wavel,s_shift=StripeM_shift))
#     matA_h.append(a[0][0])
#     matB_h.append(a[0][1])
#     matC_h.append(a[1][0])
#     matD_h.append(a[1][1])
...
# plt.figure()
# plt.plot(wavels*1E6,[abs(matA_n+matD_n)/2 for matA_n,matD_n in zip(matA,matD)])
# plt.plot(wavels*1E6,[abs(matA_n+matD_n)/2 for matA_n,matD_n in zip(matA_h,matD_h)])
# plt.xlabel("wavelength (nm)")
# plt.ylabel("stibility criterion (stable if it is smaller than 1)")
# plt.legend(['vertical','horizontal'],loc = 'upper left')
# plt.show()
# plt.figure()
max_R = []
...
plt.plot(wavels*1E6,max_R_S)
legend.append('maximun deviation with small movement '+str(StripeM_shift)+ 'mm')
# max_R_S = []
# for wavel in wavels:
#   # max_R_S.append(cavity_and_stretcher(want_to_draw=False,roundtrip = roundtrip,centerlamda=wavel,s_shift=0.1185))
...
# cavity_and_stretcher(C_radius=C_radius,want_to_draw=True,roundtrip = roundtrip,centerlamda=1030E-6,s_shift=0)
plt.show()
